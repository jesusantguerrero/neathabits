...
export async function updateRow(tableName: string, row: any) {
  const supabase = AuthState.provider.supabase
  const { data, error } = await supabase
    .from(tableName)
...
export function usePlanApi() {
  ...
  const tableName = 'scheduled_events'
  const add = async(scheduledEvent) => {
    ...
    if (scheduledEvent.labels) {
      await addRows('scheduled_events_labels', scheduledEvent.labels.map(label => ({
  ...
  const update = async(id, scheduledEvent) => {
    const supabase = AuthState.provider.supabase
    await supabase.from('scheduled_events_labels').delete().eq('event_id', id)
